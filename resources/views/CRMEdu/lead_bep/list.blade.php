...
@section('main')
    ...
    <div class="kt-container  kt-container--fluid  kt-grid__item kt-grid__item--fluid">
        <?php
        $count_leads_created = \App\CRMEdu\Models\Lead::where('admin_id', $saler_ids)
            ->where('created_at', '>=', date('Y-m-d 00:00:00', strtotime($day)))
            ->where('created_at', '<=', date('Y-m-d 23:59:59', strtotime($day)))
            ->count();
        $coun_lead_contacted_logs = \App\CRMEdu\Models\LeadContactedLog::where('admin_id', $saler_ids)
            ->where('created_at', '>=', date('Y-m-d 00:00:00', strtotime($day)))
            ->where('created_at', '<=', date('Y-m-d 23:59:59', strtotime($day)))->count();
        $count_khqt = \App\CRMEdu\Models\Lead::where('saler_ids', 'like', '%|' . $saler_ids . '|%')
        ...
        $count_co_hoi = \App\CRMEdu\Models\Lead::where('saler_ids', 'like', '%|' . $saler_ids . '|%')
