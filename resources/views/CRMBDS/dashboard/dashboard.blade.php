...
@section('main')
    ...
    if (!$bill_warning) {
        // ====|Min|======|Now|=====|Closed|======|Max|======>
        $bill_warning = \App\CRMBDS\Models\Bill::select('service_id', 'customer_id', 'id', 'created_at', 'total_price', 'exp_price', 'expiry_date', 'domain', 'auto_extend', 'saler_id')
    ...
    $tong_hd = @\App\CRMBDS\Models\Bill::whereRaw($whereRegistration)->count();


    $tong_khach = @\App\CRMBDS\Models\Bill::whereRaw($where)->select('id')->get()->count();


    $doanh_so = \App\CRMBDS\Models\Bill::whereRaw($whereRegistration)->sum('total_price');


    $doanh_thu_du_an = \App\CRMBDS\Models\Bill::whereRaw($whereRegistration)->sum('total_received');

    $phieu_thu = \App\CRMBDS\Models\BillReceipts::where('price', '>', 0)->whereRaw($whereDate)->sum('price');

    $phieu_chi = \App\CRMBDS\Models\BillReceipts::where('price', '<', 0)->whereRaw($whereDate)->sum('price');

    ?>
    <div class="kt-container  kt-container--fluid  kt-grid__item kt-grid__item--fluid">
        <div class="row">
            @include('CRMBDS.dashboard.partials.bo_loc.bo_loc_chung')
        </div>
        <div class="row">
            <div class="col-xs-12 col-md-6 order-lg-1 order-xl-1">
                @include('CRMBDS.dashboard.partials.so_lieu.so_lieu_tong_quan')
            </div>
            <div class="col-xs-12 col-md-6 order-lg-1 order-xl-1">
                @include('CRMBDS.dashboard.partials.so_lieu.chi_tiet_chi_phi')
            </div>
        </div>

        <div class="row">
            <div class="col-xs-12 col-md-6 order-lg-1 order-xl-1">
                @include('CRMBDS.dashboard.partials.xep_hang.xep_hang_dich_vu')
            </div>
            <div class="col-xs-12 col-md-6 order-lg-1 order-xl-1">
                @include('CRMBDS.dashboard.partials.xep_hang.top_sale')
            </div>
        </div>

        <div class="row">
            <div class="col-xl-12 order-xl-1">
                @include('CRMBDS.dashboard.partials.nhac_nho.hd_sap_het_han')
            </div>
        ...
        <div class="row">
            ...
            <div class="col-xl-12 order-lg-2 order-xl-1">
                @include('CRMBDS.dashboard.partials.bieu_do.hop_dong_doanh_so_khach_hang')
            </div>
        </div>
    </div>
