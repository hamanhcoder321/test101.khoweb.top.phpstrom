...
<div class="kt-portlet lich_su_hop_dong">
    ...
    <div class="kt-form">
        <div class="kt-portlet__body">
            <?php 
                $customer = \App\CRMBDS\Models\Admin::select('id', 'name', 'created_at', 'sale_id')->where('tel', $result->tel)->first();
                if (is_object($customer)) {
                    echo 'Đã tạo khách hàng: <a href="/admin/user/'.$customer->id.'" target="_blank">'.$customer->name.'</a> - '.$result->tel.'<br>';
                    $bills = \App\CRMBDS\Models\Bill::select('domain', 'total_price', 'registration_date')->where('customer_id', $customer->id)->get();
                    if (count($bills) > 0) {
