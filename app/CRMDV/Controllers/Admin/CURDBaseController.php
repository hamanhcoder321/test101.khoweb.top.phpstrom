...
class CURDBaseController extends Controller
{
    ...
    public function filterSimple($request)
    {
        ...
        if (!is_null($request->id)) {
            $where .= " AND {$table}.id = " . (int)$request->id;
        }
        ...
        foreach ($this->filter as $filter_name => $filter_option) {
            ...
            // Mặc định: column thuộc bảng chính
            $column = "{$table}.{$filter_name}";
            ...
            if (isset($filter_option['multiple'])) {
                ...
                elseif ($filter_option['query_type'] === 'from_to_date2') {
                    ...
                    if ($request->get('from_date2')) {
                        $where .= " AND {$column} >= '" .
                            date('Y-m-d 00:00:00', strtotime($request->get('from_date2'))) . "'";
                    }

                    if ($request->get('to_date2')) {
                        $where .= " AND {$column} <= '" .
                            date('Y-m-d 23:59:59', strtotime($request->get('to_date2'))) . "'";
                    }
                }

                elseif ($filter_option['query_type'] === 'filter_date') {

                    // cột động nhưng vẫn prefix bảng chính
                    $customColumn = $request->get($filter_name);

                    if ($customColumn) {
                        $customColumn = "{$table}.{$customColumn}";

                        if ($request->get($filter_name . '_from_date')) {
                            $where .= " AND {$customColumn} >= '" .
                                date('Y-m-d 00:00:00', strtotime($request->get($filter_name . '_from_date'))) . "'";
                        }

                        if ($request->get($filter_name . '_to_date')) {
                            $where .= " AND {$customColumn} <= '" .
                                date('Y-m-d 23:59:59', strtotime($request->get($filter_name . '_to_date'))) . "'";
                        }
                    }
                }
            }
    ...
    public function getAllFormFiled()
    {
        ...
        foreach ($this->module['form'] as $tab) {
            foreach ($tab as $field) {
                ...
                // Bỏ field động như field_1, field_2...
                if (strpos($field['name'], 'field_') === 0) {
                    continue;
                }
