...
// dd($whereCreated_at);
    ...
    <div class="kt-container  kt-container--fluid  kt-grid__item kt-grid__item--fluid">
        ...
        <div class="lead-chart">
            <label style="color: red;">Mục tiêu tuần1 @if(is_object($plan))
        ...
        <div class="loc-rate" style="margin-bottom: 5px;">
            ...
            @foreach ($rates as $rate)
                <a class="kt-badge kt-badge--default kt-badge--inline kt-badge--pill" href="/admin/lead?search=true&rate%5B%5D={{ $rate->id }}" style="cursor:pointer;
                        ...
                        @if(in_array('lead_float_view', $permissions))
                            <div>
                                ...
                                <a href="/admin/lead/chua-chia"
                                   class="btn {{ strpos($_SERVER['REQUEST_URI'], '/chua-chia') != false ? 'btn-primary' : 'btn-default' }}">Khách chưa chia</a>
    ...
    @if(in_array(CommonHelper::getRoleName(\Auth::guard('admin')->user()->id, 'name'), ['telesale', 'sale', 'truong_phong_sale']))
        @include('CRMBDS.lead.list.partials.nhac_nho_telesale')
    @endif
@endsection

@section('custom_head')
    <link type="text/css" rel="stylesheet" charset="UTF-8"
          href="{{ asset(config('core.admin_asset').'/css/list.css') }}?v={{ time() }}">
