...
//                    'Đã ký HĐ' => 'Đã ký HĐ',
                ...
                ['name' => 'source', 'type' => 'select2_model', 'model' => Tag::class, 'label' => 'Nguồn khách', 'group_class' => 'col-md-4', 'display_field' => 'name', 'where' => 'type="lead_source"', 'object' => 'tag', 'orderByRaw' => 'order_no desc'],
    ...
    {
        $extendFields = \DB::table('tags')->where('type', 'leads_field_extend')->orderBy('order_no')->get();
        $extend_tab = [];
        foreach ($extendFields as $field) {
            $extend_tab[] = [
                ...
                'type' => 'text',
                'label' => $field->name,
                'link' => url('admin/tag/edit/' . $field->id),
            ];
        }
        $this->module['form']['extend_tab'] = $extend_tab;
        parent::__construct();
    }
    ...
    protected $filter = [
        ...
        'source' => [
            ...
            'display_field' => 'name',
            'display_field2' => 'code',
            'model' => \App\CRMDV\Models\Tag::class,
            'object' => 'tag',
...
//        }
        ...
        $visibleColumns = explode(',', $request->input('visible_columns', ''));
        $visibleColumnKeys = array_flip(array_filter($visibleColumns));
...
//                     $query->orWhere('admin_id', \Auth::guard('admin')->user()->id);
        ...
        if (strpos($request->url(), '/doi-tac') === false) {
            $query = $query->where(function ($q) {
                // Lấy các bản ghi MÀ (cột partner không chứa ID của mình HOẶC null)
                $q->where('partner', 'NOT LIKE', '%|' . \Auth::guard('admin')->user()->id . '|%')
                    ->orWhereNull('partner');
            });
        }
...
//                ->where('dating', '<=', date('Y-m-d 23:59:59'));
    ...
    public function getIndex(Request $request)
    {
        ...
        return view('CRMDV.lead.list')->with($data);
    }
    public function multiRelease(Request $request)
    {
        try {
            $ids = $request->ids;

            if (is_array($ids) && count($ids) > 0) {
                // Cập nhật trạng thái các ID đã chọn sang "Thả nổi"
                ...
                return response()->json([
                    'status' => true,
                    'msg' => 'Đã chuyển ' . count($ids) . ' đầu mối sang trạng thái "Thả nổi" thành công!'
                ]);
            }
    ...
    public function add(Request $request)
    {
        try {
            ...
            if (!$_POST) {
                ...
                return view('CRMDV.lead.add')->with($data);
            } else if ($_POST) {
                \DB::beginTransaction();
    ...
    public function update(Request $request)
    {
        try {
            ...
            if (!$_POST) {
                ...
                $data['page_title'] = 'Thông tin khách: ' . $item->name;
                return view('CRMDV.lead.edit')->with($data);

            } else if ($_POST) {
                \DB::beginTransaction();

                // === 1. LƯU LẠI GIÁ TRỊ CŨ (ID) ===
                $old_status = $item->status;
                $old_rate_id = $item->rate; // Đây là ID
                $old_saler_ids = $item->saler_ids;
                // ==================================
                ...
                if ($validator->fails()) {
                    ...
                    // Check trùng sđt (Giữ nguyên)
                    if ($data['tel'] != $item->tel) {
                        if (Lead::where('tel', $request->tel)->where('id', '!=', $item->id)->count() > 0) {
                            CommonHelper::one_time_message('error', 'Số điện thoại '.$request->tel.' bị trùng');
                            return back()->withErrors($validator)->withInput();
                        }
                    }

                    // Xử lý Sale / Marketer ids (Giữ nguyên)
                    ...
                    // Logic đổi trạng thái tự động
                    if ($data['status'] == 'Thả nổi' && $item->status != 'Thả nổi' && in_array($item->rate, ['Đang tìm hiểu', 'Care dài', 'Quan tâm cao', 'Cơ hội'])) {
                        // Logic cũ của bạn gán chuỗi, nhưng rate thực tế là ID tag
                        // Cần tìm ID của tag "Đã từng quan tâm" nếu muốn chuẩn, hoặc gán ID tương ứng
                        // Ở đây tôi giữ nguyên logic gán chuỗi của bạn, nhưng nếu lỗi thì bạn nên gán ID tag
                    ...
                    if ($item->save()) {
                        ...
                        // === 2. GHI LOG (SỬA LẠI ĐOẠN NÀY) ===
                        $current_time = date('Y-m-d H:i:s'); // Lấy thời gian hiện tại để fix lỗi hiển thị ngày

                        // --- Log Đổi TRẠNG THÁI ---
                        if ($old_status != $item->status) {
                            $this->LeadContactedLog([
                                'title' => 'Đổi trạng thái',
                                'note' => 'Thay đổi từ: <b>' . ($old_status ?? 'Trống') . '</b> sang <b>' . $item->status . '</b>',
                                'lead_id' => $item->id,
                                'type' => 'lead',
                                'created_at' => $current_time, // Fix lỗi ngày tháng
                                'updated_at' => $current_time
                            ]);
                        }

                        // --- Log Đổi ĐÁNH GIÁ (SỬA LỖI HIỂN THỊ ID) ---
                        if ($old_rate_id != $item->rate) {
                            // Truy vấn vào bảng Tag để lấy Tên thật thay vì số ID
                            $old_rate_name = \App\Models\Tag::where('id', $old_rate_id)->value('name');
                            $new_rate_name = \App\Models\Tag::where('id', $item->rate)->value('name');

                            // Nếu không tìm thấy tên (do xóa tag hoặc null) thì hiển thị giá trị gốc
                            $txt_old = $old_rate_name ? $old_rate_name : ($old_rate_id ?? 'Chưa đánh giá');
                            $txt_new = $new_rate_name ? $new_rate_name : ($item->rate ?? 'Chưa đánh giá');

                            $this->LeadContactedLog([
                                'title' => 'Đổi đánh giá',
                                'note' => 'Thay đổi từ: <b>' . $txt_old . '</b> sang <b>' . $txt_new . '</b>',
                                'lead_id' => $item->id,
                                'type' => 'lead',
                                'created_at' => $current_time, // Fix lỗi ngày tháng
                                'updated_at' => $current_time
                        ...
                        // --- Log Đổi SALE ---
                        // --- Log Đổi SALE ---
                        if ($old_saler_ids != $item->saler_ids) {

                            // 1. Tách ID sale cũ & mới
                            $old_sale_ids = array_filter(explode('|', $old_saler_ids));
                            $new_sale_ids = array_filter(explode('|', $item->saler_ids));

                            // 2. Lấy tên sale từ bảng admin
                            $old_sale_names = \App\Models\Admin::whereIn('id', $old_sale_ids)
                                ->pluck('name')
                                ->toArray();

                            $new_sale_names = \App\Models\Admin::whereIn('id', $new_sale_ids)
                                ->pluck('name')
                                ->toArray();

                            // 3. Chuỗi hiển thị
                            $txt_old = count($old_sale_names)
                                ? implode(', ', $old_sale_names)
                                : 'Chưa có sale';

                            $txt_new = count($new_sale_names)
                                ? implode(', ', $new_sale_names)
                                : 'Chưa có sale';

                            // 4. Ghi log
                            $this->LeadContactedLog([
                                'title' => 'Đổi sale phụ trách',
                                'note'  => 'Thay đổi từ: <b>' . $txt_old . '</b> sang <b>' . $txt_new . '</b>',
                                'lead_id' => $item->id,
                                'type' => 'lead',
...
//                            $this->LeadContactedLog([
//                                'title' => 'Đổi sale phụ trách',
////                                'note' => 'Đã cập nhật người phụ trách mới',
//                                'note' => 'Thay đổi từ: <b>' . $txt_old . '</b> sang <b>' . $txt_new . '</b>',
//                                'lead_id' => $item->id,
//                                'type' => 'lead',
//                                'created_at' => $current_time,
...
//                        }
    ...
    public function view(Request $request) {
        ...
        $data = $this->getDataUpdate($request, $item);
        return view('CRMDV.lead.view')->with($data);
    }
    ...
    public function show()
    {
        ...
        $data['page_type'] = 'list';
        return view('CRMDV.lead.show')->with($data);
    }
    ...
    public function tooltipInfo(Request $r) {
        ...
        return view('CRMDV.lead.tooltip_info')->with($data);
    }
    ...
    public function sendMail() {
        ...
        if ($settings['admin_receives_mail'] == 1) {
            ...
            foreach ($admins as $admin) {
                ...
                $data = [
                    'view' => 'CRMDV.lead.emails.tien_do_cong_viec',
    ...
    public function checkExist(Request $request) {
        ...
        if ($request->has('tel')) {
            ...
            $customer = \App\CRMDV\Models\Admin::select('id', 'email', 'name', 'tel', 'created_at', 'sale_id')->where('tel', $request->tel)->first();
            if (is_object($customer)) {
...
//        dd($row);
    ...
    public function dashboard(Request $r) {
        $data['page_title'] = 'Báo cáo kinh doanh';
        $data['page_type'] = 'list';

        return view('CRMDV.lead.dashboard.dashboard', $data);
    }
}
