...
namespace App\Modules\HBBill\Controllers\Helpers;

use App\Modules\HBBill\Controllers\Admin\DomainErrorLogController;
use App\Modules\HBBill\Models\LinkErrorLogs;
...
//        echo HBBillHelper::processContentMail($camp->email_template->content); die;

        $data = [
            'sender_account' => $camp->email_account,
            'user' => $user,
            'subject' => $camp->subject,
            'content' => HBBillHelper::processContentMail(@$camp->email_template->content),
            'cc' => $cc
        ];
            ...
            foreach ($linkchecks as $item) {
                ...
                if (strpos($item->link, '.') != false) {
                    ...
                    if ($code == 0 || $code == 301 || $code == 302) {
                        ...
                        $code = HBBillHelper::reCheckLink($code, $linksDirect, $item->link, $ch);
                    }
                    ...
                    if ($code == 0) {
                        ...
                        $list[] = HBBillHelper::logs_error_link($code, $linkErrorLog);
