...
namespace App\CRMDV\Controllers\Frontend;
...
class CodeController extends Controller
{
    ...
    public function landingpage(Request $r)
    {
        ...
        foreach ($codes as $code) {
            ...
            $ldp = Landingpage::where('domain', $domain)->first();
            if (is_object($ldp)) {
//                dd($ldp, $code);
...
//            dd($codes, $type);
        ...
        return view('CRMDV.frontend.pages.code.list')->with($data);
    }
    ...
    public function wordpress(Request $r)
    {
        ...
        $this->whereRaw = "source like '%wordpress%' and image IS NOT NULL ";
    ...
    public function wordpressTrans(Request $r, $lang)
    {
        ...
        return view('CRMDV.frontend.pages.code.list_trans')->with($data);
    }

    public function appendWhereWordpress($listItem, $r) {
        // if (@$r->bill_id != null) {
        //     $listItem = $listItem->whereNotNull('bill_id');
        // } else {
        //     $listItem = $listItem->whereNull('bill_id');
        // }
        if (@$r->status != null) {
...
//        dd($listItem->get(), $r->cat_id, 'like', '%|'.$r->cat_id.'|%');
        if ($r->has('tag_id') && $r->tag_id != '') {
            $code_ids = PostTag::where('tag_id', $r->tag_id)->pluck('post_id')->toArray();
            $listItem->whereIn('id', $code_ids);
        }

        if (@$r->owned == 'Kho' || @$r->owned == '') {
            $listItem->where('owned', 'bizhostvn.com');
        } elseif (@$r->owned == 'Tất cả') {
//            dd($listItem->get());
        } else {
    ...
    public function app(Request $r)
    {
        ...
        return view('CRMDV.frontend.pages.code.list_app')->with($data);
    }
    ...
    public function appDetail(Request $r, $id) {
        ...
        return view('CRMDV.frontend.pages.code.detail_app')->with($data);
    }
...
//                    dd($request->all());
    ...
    public function wordpressDemo($id) {
        ...
        return view('CRMDV.frontend.pages.code.detail_wp')->with($data);
    }

    public function showForm()
    {
        return view('CRMDV.lead.form-iframe.form1');
    }

    public function showForm2()
    {
        return view('CRMDV.lead.form-iframe.form2');
    }
