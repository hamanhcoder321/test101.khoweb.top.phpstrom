...
//            ['name' => 'rate_content', 'type' => 'custom', 'td' => 'CRMDV.dhbill.list.td.rate_content', 'label' => 'Đánh giá', 'sort' => true],
            ...
            ['name' => 'auto_extend', 'type' => 'select', 'options' => [
                0 => 'Không',
                1 => 'Có'
            ], 'label' => 'Duy trì', 'sort' => true
            ],
...
//        dd((float) @$diem_dh[14]);
    ...
    public function appendWhere($query, $request)
    {
        ...
        if(@$request->progress == 'Bỏ') {
            ...
            //  Lọc dự án khách xác nhận xong / tạm dừng / kết thúc
            $bill_ids = BillProgress::whereIn('status', [
                'Khách xác nhận xong', 'Tạm dừng', 'Kết thúc', 'Bỏ', NULL
            ])
                ->pluck('bill_id')->toArray();
            $query = $query->whereNotIn('id', $bill_ids);
        ...
        if (@$request->service_ids != null) {
            $query = $query->where(function ($query) use ($request) {
                foreach ($request->service_ids as $service_id) {
                    $query->orWhere('service_id', $service_id);
                }
            });
        }
    ...
    public function update(Request $request)
    {
        try {
            ...
            if ($billProgress == null) {
                $billProgress = new BillProgress();
                $billProgress->bill_id = $item->id;
                $billProgress->save();
            }
            ...
            if ($billProgress != null && !in_array(@$billProgress->progress_status, ['Kết thúc', 'Tạm dừng', 'Bỏ']) && in_array($request->progress_status, ['Kết thúc', 'Tạm dừng', 'Bỏ'])) {
...
//            }
    ...
    public function changeStatus(Request $r) {
        BillProgress::where(function($q) {
            $q->whereNotIn('status', ['Kết thúc', 'Bỏ', 'Tạm dừng'])
                ->orWhereNull('status');
        })->whereIn('bill_id', explode(',', $r->bill_ids))->update(['status' => $r->status]);
