...
@section('main')
    ...
    <div class="col-xs-12 menu-document">
        <ul class="nav">
            ...
            <?php 
                $categories = \App\CRMWoo\Models\Category::select('id', 'name')->where('status', 1)->where('type', 1)
            ...
            @foreach($categories as $cat)
                <?php 
                    $cat_childs = \App\CRMWoo\Models\Category::select('id', 'name')->where('status', 1)->where('type', 1)
            ...
            <li class="nav-item">
                ...
                <ul class="dropdown-menu">
                    @foreach($cat_childs as $cat_child)
                        <?php 
                            $cat3_childs = \App\CRMWoo\Models\Category::select('id', 'name')->where('status', 1)->where('type', 1)
                                            ->where('role_ids', 'LIKE', '%|'.@\App\Models\RoleAdmin::where('admin_id', \Auth::guard('admin')->user()->id)->first()->role_id.'|%')
