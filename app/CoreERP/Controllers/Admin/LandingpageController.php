    ...
    public function updateToBill() {
        ...
        foreach ($ldps as $ldp) {
            ...
            if ($count_bill == 0) {
                ...
                // $bill->customer_tel = @$ldp->user->tel;
                // $bill->customer_name = @$ldp->user->name;
                // $bill->customer_email = @$ldp->user->email;
                // $bill->customer_address = @$ldp->user->address;
                $bill->status = 1;
                $bill->service_id = @$ldp_service->id;
                $bill->domain = @$ldp->domain;
                $bill->exp_price = $priceExpiry;
                $bill->auto_extend = 1;
                $bill->company_id = @$ldp->user->last_company_id;
                $bill->save();
                $count_bill_create ++;
            }
