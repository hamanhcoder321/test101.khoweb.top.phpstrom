...
use App\CRMWoo\Controllers\Admin\CURDBaseController;
use Validator;
...
class ReceiptPaymentController extends CURDBaseController
{
    ...
    protected $module = [
        ...
        'form' => [
            'general_tab' => [
                ...
                ], 'class' => 'required', 'label' => 'Tài khoản', 'group_class' => 'col-md-4'],
                ['name' => 'price', 'type' => 'price_vi', 'class' => 'required','label' => 'Số tiền giao dịch', 'group_class' => 'col-md-6'],
                ['name' => 'note', 'type' => 'textarea', 'class' => 'required', 'label' => 'Lý do'],
                ['name' => 'image', 'type' => 'file_image', 'class' => '', 'label' => 'Ảnh bằng chứng'],
                ['name' => 'employees', 'type' => 'text', 'class' => '','label' => 'Tên người thực hiện'],
            ],
        ],
    ];
    ...
    {
//        $price = BillReceipts::where('bill_id', 1)
//            ->where('status', 1)
//            ->sum('price');
//        Bill::where('id', 1)->update(['total_received' => $price]);
//        dd($price);

        $data = $this->getDataList($request);

        return view('CRMWoo.receipt_payment.list')->with($data);
    }
    ...
    public function add(Request $request)
    {
        try {
            if (!$_POST) {
                ...
                return view('CRMWoo.receipt_payment.add')->with($data);
            } else if ($_POST) {
...
//                    'value.required' => 'Bắt buộc phải nhập giá trị',
    ...
    public function update(Request $request)
    {
        ...
        if (!$_POST) {
            ...
            return view('CRMWoo.receipt_payment.edit')->with($data);
        } else if ($_POST) {
            $validator = Validator::make($request->all(), [
                // 'value' => 'required'
            ], [
