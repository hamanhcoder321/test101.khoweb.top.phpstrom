@if(!isset($_GET['ajax-load']))
    <script>
        $(document).ready(function () {
            $.ajax({
                url: '/admin/dashboard/ajax/load-khoi?ajax-load=true&file=HBBill.lead.dashboard.partials.tuong_tac_gan_day',
                type: 'GET',
                data: {
...
@else
    ...
    <table class="table table-striped">
        ...
        <tbody class="kt-datatable__body ps ps--active-y">
            ...
            ?>
            <?php
                $lead_contacted_log = \App\Modules\HBBill\Models\LeadContactedLog::where('created_at', '>=', date('Y-m-d 00:00:00', strtotime('-2 days')))
                    ->where('type', 'lead')
                    ->orderBy('id', 'desc')
                    ->get();
                $thong_ke = [];
