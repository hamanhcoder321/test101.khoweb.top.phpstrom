...
use Session;
use Validator;
use App\CRMWoo\Models\DailyWorkReport;
...
class AdminController extends CURDBaseController
{
    ...
    protected $module = [
        ...
        'form' => [
            'general_tab' => [
                ...
                ['name' => 'email', 'type' => 'custom', 'field' => 'CRMWoo.admin.form.email', 'class' => 'required', 'label' => 'admin.email', 'group_class' => 'col-md-3'],
                ['name' => 'tel', 'type' => 'custom', 'class' => 'required', 'field' => 'CRMWoo.admin.form.tel', 'label' => 'admin.phone', 'group_class' => 'col-md-2'],
                ['name' => 'code', 'type' => 'custom', 'class' => 'required', 'field' => 'CRMWoo.admin.form.code', 'label' => 'Mã nhân viên', 'group_class' => 'col-md-2'],
                ...
                /*['name' => 'role_id', 'type' => 'custom', 'field' => 'CRMWoo.admin.partials.select_role', 'label' => 'Quyền', 'class' => 'required', 'model' => \App\Models\Roles::class, 'display_field' => 'display_name', 'group_class' => 'col-md-6'],*/
    ...
    public function getIndex(Request $request)
    {
        ...
        return view('CRMWoo.admin.list')->with($data);
    }

    public function appendWhere($query, $request)
    {
        if($request->status == null) {
            //  Khi ko dùng bộ lọc trạng thái thì mặc định ko hiện ra các thành viên trạng thái huỷ kick hoạt
    ...
    public function add(Request $request)
    {
        try {
            ...
            if (!$_POST) {
                ...
                return view('CRMWoo.admin.add')->with($data);
            } else if ($_POST) {
    ...
    public function update(Request $request)
    {
        try {
            ...
            if (!$_POST) {
                ...
                return view('CRMWoo.admin.edit')->with($data);
            } else if ($_POST) {
...
//                }
    ...
    public function checkExist(Request $request) {
        if ($request->has('email')) {
            ...
            if (is_object($admin)) {
                return response()->json([
                    'status' => false,
                    'html' => 'Email này đã được tạo cho <a  target="_blank" href="/admin/admin/'.$admin->id.'">'.$admin->name .' - email: ' . $admin->email.' - sđt: ' . $admin->tel.' - mã: ' . $admin->code.'</a>'
                ]);
            }
        }
        if ($request->has('tel')) {
            ...
            if (is_object($admin)) {
                return response()->json([
                    'status' => false,
                    'html' => 'SĐT này đã được tạo cho <a target="_blank" href="/admin/admin/'.$admin->id.'">'.$admin->name .' - email: ' . $admin->email.' - sđt: ' . $admin->tel.' - mã: ' . $admin->code.'</a>'
                ]);
            }
        }
        if ($request->has('code')) {
    ...
    public function hieuSuatCongViec() {
        ...
        return view('CRMWoo.admin.hieu_suat_cong_viec')->with($data);
    }
    ...
    public function updateDailyWorkReport() {
        ...
        foreach($admins as $admin) {
            ...
            $count_leads_created = \App\CRMWoo\Models\Lead::whereRaw($whereSale)
                ->whereRaw($whereCreated_at)
                ->count();

            $coun_lead_contacted_logs = \App\CRMWoo\Models\LeadContactedLog::whereRaw($whereSale)
                ->whereRaw($whereCreated_at)
                ->whereNotIn('note', ['Không nghe', 'Sai số'])->count();

            $count_khqt = \App\CRMWoo\Models\Lead::whereRaw($whereLikeSale)
            ...
            $quanTamMoi = \App\CRMWoo\Models\LeadContactedLog::whereRaw($whereSale)
                ->where('created_at', '>', date('Y-m-d 00:00:00'))
                ->where('type', 'lead_quan_tam_lai')->count();

            $count_khqt_cao = \App\CRMWoo\Models\Lead::whereRaw($whereLikeSale)
