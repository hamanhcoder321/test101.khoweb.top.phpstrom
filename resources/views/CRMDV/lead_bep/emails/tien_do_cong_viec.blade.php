...
$lead_chua_log = \App\CRMDV\Models\Lead::select('id', 'tel', 'name', 'created_at')
					->where('saler_ids', 'like', '%|' . $admin_id . '|%')
					->whereIn('status', ['Đang chăm sóc'])
					->where('contacted_log_last', null)
					->get();

$lead_sap_tha_noi = \App\CRMDV\Models\Lead::select('id', 'tel', 'name', 'created_at', 'contacted_log_last')
					->where('saler_ids', 'like', '%|' . $admin_id . '|%')
					->whereIn('status', ['Đang chăm sóc'])
