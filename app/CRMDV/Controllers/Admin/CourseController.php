        ...
        if ($request->category_id == 0) {
            ...
            if (!CommonHelper::has_permission(\Auth::guard('admin')->user()->id, 'course')) {
                ...
                $cat_ids = $cat_ids->where(function ($query) {
                    $query->orWhere('role_ids', 'LIKE', '%|'.@RoleAdmin::where('admin_id', \Auth::guard('admin')->user()->id)->first()->role_id.'|%');
            ...
            if (!CommonHelper::has_permission(\Auth::guard('admin')->user()->id, 'course')) {
                //  nếu ko có quyền quản lý tài liệu thì lọc theo nhóm quyền
                $cat_ids = $cat_ids->where('role_ids', 'LIKE', '%|'.@RoleAdmin::where('admin_id', \Auth::guard('admin')->user()->id)->first()->role_id.'|%');
            }
