...
?>
    <div class="kt-container  kt-container--fluid  kt-grid__item kt-grid__item--fluid">
        <?php
        $count_leads_created = \Modules\WebBill\Models\Lead::where('admin_id', $saler_ids)
                    ->where('created_at', '>=', date('Y-m-d 00:00:00', strtotime($day)))
                    ->where('created_at', '<=', date('Y-m-d 23:59:59', strtotime($day)))
                    ->count();
        $coun_lead_contacted_logs = \Modules\WebBill\Models\LeadContactedLog::where('admin_id', $saler_ids)
                ->where('created_at', '>=', date('Y-m-d 00:00:00', strtotime($day)))
                ->where('created_at', '<=', date('Y-m-d 23:59:59', strtotime($day)))->count();
        $count_khqt = \Modules\WebBill\Models\Lead::where('saler_ids', 'like', '%|' . $saler_ids . '|%')
        ...
        $count_co_hoi = \Modules\WebBill\Models\Lead::where('saler_ids', 'like', '%|' . $saler_ids . '|%')
                        ...
                        @foreach($listItem as $item)
                            <tr data-row="0" class="kt-datatable__row" style="left: 0px;">
                                ...
                                <td class="kt-datatable__cell--center kt-datatable__cell kt-datatable__cell--check"
                                    ...
                                    <div style="width: 500px;    left: 356px;" id="tooltip-info-{{$field['name']}}" class="dropdown-menu div-tooltip_info"
