...
var loadConfigFile = function (type) {
	...
	if(type == 'user') {
		var url = '/public/filemanager/scripts/filemanager.config.js';
	} else {
		var url = '/public/filemanager/scripts/filemanager.config.js.default'
	}
...
if($.urlParam('langCode') != 0 && file_exists ('/public/filemanager/scripts/languages/'  + $.urlParam('langCode') + '.js')) config.options.culture = $.urlParam('langCode');

var lg = [];
$.ajax({
  url: '/public/filemanager/scripts/languages/'  + config.options.culture + '.js',
  async: false,
  dataType: 'json',
...
$(function(){
	...
	if(config.edit.enabled) {
		loadCSS('/public/filemanager/scripts/CodeMirror/lib/codemirror.css');
		loadCSS('/public/filemanager/scripts/CodeMirror/theme/' + config.edit.theme + '.css');
		loadJS('/public/filemanager/scripts/CodeMirror/lib/codemirror.js');
		loadJS('/public/filemanager/scripts/CodeMirror/addon/selection/active-line.js');
		loadJS('/public/filemanager/scripts/CodeMirror/dynamic-mode.js');
	}

	if(!config.options.fileRoot) {
		fileRoot = '/public/' + document.location.pathname.substring(1, document.location.pathname.lastIndexOf('/') + 1) + 'userfiles/';
	} else {
	...
	/** load searchbox */
	if(config.options.searchBox === true)  {
		loadJS("/public/filemanager/scripts/filemanager.liveSearch.min.js");
	} else {
		$('#search').remove();
	}
