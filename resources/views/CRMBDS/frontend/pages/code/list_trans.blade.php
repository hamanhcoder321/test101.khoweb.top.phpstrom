...
<div class="kho-code theme-container">
    <div class="kho-code-form">
        <form method="GET" action="" id="form-search">
    		...
    		<input type="hidden" name="_x_tr_sl" value="vi">
    		<input type="hidden" name="_x_tr_tl" value="{{ @$lang }}">
    		<input type="hidden" name="_x_tr_hl" value="vi">
            ...
            <div class="col-md-3">
                <?php
                $cats = \App\CRMBDS\Models\Category::select('id', 'name')->where('type', 10)->where('status', 1)->orderBy('name', 'asc')->get();
                ?>
            ...
            <div class="col-md-3">
                <?php
                $tag_ids = \App\CRMBDS\Models\PostTag::where('multi_cat', 'like', '%|'.@$_GET['cat_id'].'|%')->groupBy('tag_id')->pluck('tag_id')->toArray();
                $tags = \App\CRMBDS\Models\Tag::whereIn('id', $tag_ids)->orderBy('name', 'asc')->get();
                ?>
                        ...
                        <div class="theme-links" style="top: 70%;">
                            ...
                            @else
                                @if(strpos($_SERVER['REQUEST_URI'], 'landingpage') !== false
                                    || strpos($_SERVER['REQUEST_URI'], 'ldp') !== false)
                                	<a rel="nofollow" target="_blank" class="demo"
                                   		href="/kho-giao-dien/landingpage/view?link={{$item->link}}">Xem Demo
                                	</a>
                                @else

                                	<?php 
                                	$domain_data = explode('://', $item->link)[1];
                                	$domain = explode('/', $domain_data)[0];
                                	$duong_dan = str_replace($domain, '', $domain_data);
                                	$link = 'https://'.str_replace('.', '-', $domain).'.translate.goog'.$duong_dan.'?_x_tr_sl=vi&_x_tr_tl='.$lang.'&_x_tr_hl=vi&_x_tr_pto=wapp'
                                	?>
                                    <a rel="nofollow" target="_blank" class="demo"
	                               href="{{ $item->link }}" translate="no">Xem
	                                Demo</a>
                                @endif
                            @endif
