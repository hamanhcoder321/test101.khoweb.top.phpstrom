...
use App\CRMDV\Models\Tag as tagg;
class AdminController extends CURDBaseController
{
    ...
    protected $module = [
        ...
        'list' => [
            ...
            ['name' => 'room_id', 'type' => 'text', 'label' => 'Phòng', 'sort' => true],
        ...
        'form' => [
            'general_tab' => [
                ...
                ['name' => 'birthday', 'type' => 'date', 'class' => '', 'label' => 'Ngày sinh', 'group_class' => 'col-md-3'],
            ...
            'more_info_tab' => [
                ...
                ['name' => 'room_id', 'type' => 'select2_model', 'label' => 'Phòng ban', 'model' => tagg::class,
                    'where' => 'type="phong_ban"', 'object' => 'tag', 'display_field' => 'name', 'group_class' => 'col-md-12'],
                ['name' => 'work_time', 'type' => 'select', 'options' => [
                    '' => '',
                    1 => 'Fulltime',
                    2 => 'Parttime',
    ...
    {
        try {
            \App\Models\Admin::checkMaxActiveAdmins();
            if (!$_POST) {
...
//                }
    ...
    public function sinhNhatNhanSu(Request $request)
    {
        ...
        $months = [
            'previous' => ($currentMonth == 1) ? 12 : $currentMonth - 1,
            'current'  => $currentMonth,
            'next'     => ($currentMonth == 12) ? 1 : $currentMonth + 1,
        ];

        $data = DB::table('admin')
            ->selectRaw('MONTH(birthday) as month, name, code, tel, birthday, room_id')
            ->whereNotNull('birthday')
            ->where('status', 1)
            ->whereRaw('MONTH(birthday) IN (?, ?, ?)', [
                $months['previous'],
                $months['current'],
                $months['next']
            ])
            ->orderByRaw("FIELD(MONTH(birthday), {$months['previous']}, {$months['current']}, {$months['next']})")
            ->orderByRaw('DAY(birthday)')
            ->get();

        return view('CRMDV.admin.sinh_nhat_nhan_su', compact('data'));
    }
}
