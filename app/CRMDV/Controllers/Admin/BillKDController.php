...
class BillKDController extends CURDBaseController
{
...
//            ['name' => 'count_product', 'type' => 'custom', 'td' => 'CRMDV.list.td.count_product', 'label' => 'Tổng SP'],
        ...
        'form' => [
            'general_tab' => [
                ...
                ['name' => 'link_hd', 'des' => 'Link google docs', 'type' => 'text', 'label' => 'Link hợp đồng', 'class' => '', 'group_class' => 'col-md-3'],
        ...
        if (@$r->quick_search != '') {
            $listItem = $listItem->where(function ($query) use ($r) {
                foreach (explode(',', $this->quick_search['fields']) as $field) {
                    $query->orWhere(trim($field), '=', $r->quick_search);    //  truy vấn các tin thuộc các danh mục con của danh mục hiện tại
                }
...
//                dd($customer_ids);
    ...
    public function getIndex(Request $request)
    {
        ...
        return view('CRMDV.bill-kd.list')->with($data);
    }
    ...
    public function getDataList(Request $request) {
        $isSearching =
            $request->hasAny(array_keys($this->filter)) ||
            $request->filled('quick_search') ||
            $request->filled('search');

        if (!$isSearching && !$request->has('export')) {
            $emptyPaginator = new LengthAwarePaginator(
                collect([]),
                0,
                $request->get('limit', $this->limit_default),
                $request->get('page', 1),
            ...
            return [
                ...
                'listItem' => $emptyPaginator,
                'limit' => $request->get('limit', $this->limit_default),
                'page' => $request->get('page', 1),
                'param_url' => $request->all(),
                'module' => $this->module,
                'quick_search' => $this->quick_search,
                'filter' => $this->filter,
                'page_title' => $this->module['label'],
                'page_type' => 'list',
            ];
        }
...
//        }
        ...
        if($request->has('custom') && $request->custom != '') {
            if ($request->custom == 'Chưa thanh toán hết') {
                ...
                $query = $query->whereColumn('total_price_contract', '!=', 'total_received')
                            ->whereNotIn('id', $hd_bo_ids); //   ko truy vấn các hợp đồng bỏ
            }
        }

        if ($request->has('search') && $request->search != '') {
            $query = $query->where('domain', $request->search);
        }
    ...
    public function update(Request $request)
    {
        try {
            ...
            if (!$_POST) {
                ...
                return view('CRMDV.bill-kd.edit')->with($data);
            }
...
//        $data['bill'] = Lead::find($r->id);
        return view('CRMDV.bill.tooltip_info')->with(['bill_id' => $r->id]);
    }
}
