                                ...
                                <table class="table table-striped">
                                    ...
                                    <tbody class="kt-datatable__body ps ps--active-y">
                                    	...
                                    	$timekeepers = \App\CRMDV\Models\Timekeeper::leftJoin('admin', 'timekeeper.may_cham_cong_id', '=', 'admin.may_cham_cong_id')
                                    					->select('admin.id as ad_id', 'admin.name as ad_name', 'admin.tel as ad_tel', 'admin.code as ad_code', 'admin.room_id',
                                                            'admin.may_cham_cong_id', 'admin.image as ad_image', 'admin.tknh_image as ad_tknh_image', 'timekeeper.*')
                                    	...
                                    	foreach ($timekeepers as $timekeeper) {
                                            ...
                                            if (!isset($admins[$timekeeper->ad_id])) {
                                                ...
                                                $admins[$timekeeper->ad_id] = [
                                                    ...
                                                    'image' => $timekeeper->ad_image,
                                                    'tknh_image' => $timekeeper->ad_tknh_image,
                                                ];
                                            }
                                        ...
                                        foreach ($timekeepers as $tk) {
                                            $key = $tk->ad_id . '_' . date('Y-m-d', strtotime($tk->time));
                                            $groupedTimekeepers[$key][] = $tk;
                                        }

                                        foreach ($groupedTimekeepers as $group) {
                                            $first = reset($group); // lần đầu chấm
                                            $last  = end($group);   // lần cuối chấm

                                            $admin_id = $first->ad_id;
                                            if (!isset($admins[$admin_id])) continue;

                                            // Chuẩn bị giờ chuẩn
                                            $gio_ve_sang  = $cau_hinh['gio_ve_sang'] ?? '12:00';
                                            $gio_ve_chieu = $cau_hinh['gio_ve_chieu'] ?? '17:30';

                                            $gio_cham_cong_cuoi = date('H:i', strtotime($last->time));

                                            // Xác định buổi của lần chấm cuối
                                            $gio_cham_cong_cuoi_timestamp = strtotime($last->time);
                                            $gio_13h_timestamp = strtotime(date('Y-m-d 13:00:00', $gio_cham_cong_cuoi_timestamp));

                                            if ($gio_cham_cong_cuoi_timestamp >= $gio_13h_timestamp) {
                                                // Lần chấm cuối là buổi chiều => so sánh với giờ về chiều chuẩn
                                                if ($gio_cham_cong_cuoi < $gio_ve_chieu) {
                                                    $admins[$admin_id]['ve_som'] += 1;
                                                }
                                            } else {
                                                // Lần chấm cuối là buổi sáng => so sánh với giờ về sáng chuẩn
                                                if ($gio_cham_cong_cuoi < $gio_ve_sang) {
                                                    $admins[$admin_id]['ve_som'] += 1;
                                                }
                                            }
                                        }
                                        ...
                                        @foreach($admins as $key => $value)
                                            <tr data-admin_id="{{ $value['admin_id'] }}">
                                                ...
                                                <td><img data-src="{{ CommonHelper::getUrlImageThumb($value['tknh_image'], 80, 80) }}" class="file_image_thumb lazy" title="CLick để phóng to ảnh" style="cursor: pointer; max-width: 100px;"></td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                                </table>
