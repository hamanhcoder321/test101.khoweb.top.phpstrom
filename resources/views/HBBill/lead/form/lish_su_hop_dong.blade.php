<div class="kt-portlet">
    ...
    <div class="kt-form">
        <div class="kt-portlet__body">
            <?php 
                $customer = \App\Modules\HBBill\Models\Admin::select('id', 'name', 'created_at', 'sale_id')->where('tel', $result->tel)->first();
                if (is_object($customer)) {
                    echo 'Đã tạo khách hàng: <a href="/admin/customer/'.$customer->id.'" target="_blank">'.$customer->name.'</a><br>';
                    $bills = \App\Modules\HBBill\Models\Bill::select('domain', 'total_price', 'registration_date')->where('customer_id', $customer->id)->get();
                    if (count($bills) > 0) {
