...
//            $this->hasTooManyLoginAttempts($request);
        ...
        if (\Auth::guard('admin')->attempt(['email' => trim($request['email']), 'password' => trim($request['password'])], true)) {
            ...
            $this->luuDangNhapMotLan($request);
...
//            $this->attemptLogin5Shot($request['email']);
    ...
    protected function luuDangNhapMotLan(Request $request)
    {
        $payload = base64_encode(serialize(LaravelSession::all()));



    \App\Models\Sessions::create([
...
public function logoutAllDevise(){
    $id=\Auth::guard('admin')->id();
    $currentSessionId = request()->session()->getId();


  \App\Models\Sessions::where('user_id', $id)
        ->where('ip_address', '!=',  request()->ip())
        ->delete();

//  dd($abc);

    CommonHelper::one_time_message('success', 'Đăng xuất thành công!');
