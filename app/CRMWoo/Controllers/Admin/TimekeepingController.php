...
class TimekeepingController extends CURDBaseController
{
    ...
    protected $module = [
        ...
        'label' => 'Chấm công',
        'modal' => '\App\CRMWoo\Models\Timekeeping',
        'list' => [
            ['name' => 'day', 'type' => 'date_vi', 'label' => 'Ngày', 'sort' => true],
            ['name' => 'time', 'type' => 'text', 'label' => 'Thời gian làm', 'sort' => true],
            ['name' => 'created_at', 'type' => 'datetime_vi', 'label' => 'Tạo lúc', 'sort' => true],
            ['name' => 'status', 'type' => 'status', 'label' => 'Thanh toán', 'sort' => true],
        ],
        'form' => [
            'general_tab' => [
            ...
            'tab_2' => [
                ...
                ['name' => 'saler_id', 'type' => 'custom', 'field' => 'CRMWoo.timekeeping.form.fields.select_admin', 'label' => 'Sale', 'model' => Admin::class, 'object' => 'admin', 'display_field' => 'name', 'display_field2' => 'code', 'class' => 'required'],
                ['name' => 'job_other', 'type' => 'textarea', 'label' => 'Công việc khác', 'group_class' => 'col-md-12', 'inner' => 'rows=15'],
    ...
    public function getIndex(Request $request)
    {
        ...
        return view('CRMWoo.timekeeping.list')->with($data);
    }
    ...
    public function add(Request $request)
    {
        try {
            ...
            if (!$_POST) {
                ...
                return view('CRMWoo.timekeeping.add')->with($data);
            } else if ($_POST) {
                ...
                if ($validator->fails()) {
                    ...
                    $logs = [];
                    $lead_contacted_logs = \App\CRMWoo\Models\LeadContactedLog::select('lead_id', 'note', 'created_at')->where('admin_id', \Auth::guard('admin')->user()->id)
                    ...
                    $leads = \App\CRMWoo\Models\Lead::select('tel', 'name', 'created_at')->where('admin_id', \Auth::guard('admin')->user()->id)
                        ->where('created_at', '>=', date('Y-m-d 00:00:00', strtotime($data['day'])))
                        ->where('created_at', '<=', date('Y-m-d 23:59:59', strtotime($data['day'])))->orderBy('id', 'asc')->get();
                    foreach($leads as $v) {
    ...
    public function update(Request $request)
    {
        try {
            ...
            if (!$_POST) {
                ...
                return view('CRMWoo.timekeeping.edit')->with($data);
            } else if ($_POST) {
    ...
    public function view(Request $request) {
        ...
        $data = $this->getDataUpdate($request, $item);
        return view('CRMWoo.timekeeping.view')->with($data);
    }
    ...
    public function show()
    {
        ...
        $data['page_type'] = 'list';
        return view('CRMWoo.timekeeping.show')->with($data);
    }
    ...
    public function tooltipInfo(Request $r) {
        ...
        return view('CRMWoo.timekeeping.tooltip_info')->with($data);
    }
    ...
    public function sendMail() {
        ...
        if ($settings['admin_receives_mail'] == 1) {
            ...
            foreach ($admins as $admin) {
                ...
                $data = [
                    'view' => 'CRMWoo.timekeeping.emails.tien_do_cong_viec',
