...
// dd($whereCreated_at);
    ...
    <div class="kt-container  kt-container--fluid  kt-grid__item kt-grid__item--fluid">
        ...
        $count_khqt_cao = \App\CRMWoo\Models\Lead::whereRaw($whereLikeSale)
            ...
            // ->where('created_at', '<=', date('Y-'.date('m', strtotime($day)).'-t 23:59:59', strtotime($day)))
            ->count();
        $count_co_hoi = \App\CRMWoo\Models\Lead::whereRaw($whereLikeSale)
        ...
        $plan = \App\CRMWoo\Models\Plan::whereRaw($whereSale)->orderBy('id', 'desc')->first();
        $ds_tuan = \App\CRMWoo\Models\Bill::whereRaw($whereSaler)
            ->where('registration_date', '>=', date('Y-m-d 00:00:00', strtotime('-' . date('w') . ' days')))
            ->sum('total_price');
        $ds_thang = \App\CRMWoo\Models\Bill::whereRaw($whereSaler)
            ->where('registration_date', '>=', date('Y-m-01 00:00:00', time()))
            ->sum('total_price');
        $hd = \App\CRMWoo\Models\Bill::whereRaw($whereSaler)
            ->where('registration_date', '>=', date('Y-m-d 00:00:00', strtotime('-' . date('w') . ' days')))
            ->count();
