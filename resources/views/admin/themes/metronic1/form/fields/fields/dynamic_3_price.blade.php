<fieldset id="buildyourform-{{ $field['name'] }}" class="buildyourform {{ @$field['class'] }}">
    ...
    @foreach($data as $v)
        <div class="fieldwrapper">
            <div class="col-xs-3"><input type="date" class="form-control fieldname" name="{{ $field['name'] }}_key[]"
                                         value="{{ $v->key }}"
                                         placeholder="{{ @$field['cols'][0] }}"></div>
            <div class="col-xs-3"><input type="number" class="form-control fieldValue" value="{{ $v->value }}"
...
<script>
    $(document).ready(function () {
        $("#add-{{ $field['name'] }}").click(function () {
            ...
            var fKey = $('<div class="col-xs-3"><input type="date" class="form-control fieldname" name="{{ $field["name"] }}_key[]" placeholder="{{ @$field['cols'][0] }}"/></div>');
            var fValue = $('<div class="col-xs-3"><input type="number" class="form-control fieldValue" name="{{ $field["name"] }}_value[]" placeholder="{{ @$field['cols'][1] }}"/></div>');
