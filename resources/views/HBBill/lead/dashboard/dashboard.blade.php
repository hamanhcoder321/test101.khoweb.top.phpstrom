...
@section('main')
    ...
    $tong_hd = @\App\Modules\HBBill\Models\Bill::whereRaw($whereRegistration)->count();
    $tong_khach = @\App\Modules\HBBill\Models\Bill::whereRaw($where)->select('id')->get()->count();
    $doanh_so = \App\Modules\HBBill\Models\Bill::whereRaw($whereRegistration)->sum('total_price');
    $doanh_thu_du_an = \App\Modules\HBBill\Models\Bill::whereRaw($whereRegistration)->sum('total_received');

    $phieu_thu = \App\Modules\HBBill\Models\BillReceipts::where('price', '>', 0)->whereRaw($whereDate)->sum('price');

    $phieu_chi = \App\Modules\HBBill\Models\BillReceipts::where('price', '<', 0)->whereRaw($whereDate)->sum('price');

    ?>
    <div class="kt-container  kt-container--fluid  kt-grid__item kt-grid__item--fluid">
        <div class="row">
            @if(in_array(CommonHelper::getRoleName(\Auth::guard('admin')->user()->id, 'name'), ['super_admin', ]))
                @include('HBBill.dashboard.partials.bo_loc.bo_loc_chung')
            @endif
        </div>
...
{{--                    @include('CRMDV.dashboard.partials.xep_hang.top_sale')--}}
                ...
                @endif
                    ...
                    @include('HBBill.lead.dashboard.partials.tuong_tac_gan_day')
